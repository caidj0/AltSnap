...
static LRESULT CALLBACK MenuWindowProc(HWND hwnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam);
// Timer messages
...
static int init_movement_and_actions(POINT pt, HWND hwnd, enum action action,
                                     int button);
...
static struct config {
    ...
    struct {
        enum action  // Up to 20 BUTTONS!!!
            LMB[NACPB],
            RMB[NACPB], MMB[NACPB], MB4[NACPB], MB5[NACPB], MB6[NACPB],
            MB7[NACPB], MB8[NACPB], MB9[NACPB], MB10[NACPB], MB11[NACPB],
            MB12[NACPB], MB13[NACPB], MB14[NACPB], MB15[NACPB], MB16[NACPB],
            MB17[NACPB], MB18[NACPB], MB19[NACPB], MB20[NACPB], Scroll[NACPB],
            HScroll[NACPB];  // Plus vertical and horizontal wheels
    } Mouse;
...
static const struct OptionListItem Performance_uchars[] = {
    {"FullWin", 2},    {"TransWinOpacity", 0}, {"RefreshRate", 0},
    {"RezTimer", 0},   {"PinRate", 32},        {"MoveRate", 2},
    {"ResizeRate", 4},
};
// [Input]
static const struct OptionListItem Input_uchars[] = {
    {"TTBActions", 0},    {"KeyCombo", 0},       {"ScrollLockState", 0},
    {"LongClickMove", 0}, {"UniKeyHoldMenu", 0},
};
// [Zones]
static const struct OptionListItem Zones_uchars[] = {
    {"UseZones", 0},  {"ShowZonesPrevw", 1}, {"ZonesPrevwOpacity", 161},
    {"ZSnapMode", 0}, {"LayoutNumber", 0},   {"InterZone", 32},
#ifdef WIN64
    {"FancyZone", 0},
#endif
};
...
static pure int CLAMPW(int width) {
    return CLAMP(state.mmi.Min.x, width, state.mmi.Max.x);
}
static pure int CLAMPH(int height) {
    return CLAMP(state.mmi.Min.y, height, state.mmi.Max.y);
}
static pure int ISCLAMPEDW(int x) {
    return state.mmi.Min.x <= x && x <= state.mmi.Max.x;
}
static pure int ISCLAMPEDH(int y) {
    return state.mmi.Min.y <= y && y <= state.mmi.Max.y;
}
...
static pure int blacklisted(HWND hwnd, const struct blacklist *list) {
    ...
    if (!hwnd || !list->length || !list->items) return 0;
    // If the first element is *:*|* (NULL:NULL|NULL)then we are in whitelist
    // mode mode = 1 => blacklist, mode = 0 => whitelist;
    UCHAR mode = list->items[0].classname || list->items[0].title ||
    ...
    for (; i < list->length; i++) {
        if (!lstrcmp_star(classname, list->items[i].classname) &&
            ...
            (!list->items[i].exename ||
             !lstrcmpi(exename, list->items[i].exename))) {
            return mode;
        }
    }
    return !mode;
...
static int IsFullscreenF(HWND hwnd, const RECT *wnd, const RECT *fmon) {
    ...
    return ((style & WS_CAPTION) != WS_CAPTION) && EqualRect(wnd, fmon);
}
...
static void NotifySizeMoveStaEnd(HWND hwnd, UCHAR sta) {
    ...
    if (conf.NotifyWinEvent)
        NotifyWinEvent(
            sta ? EVENT_SYSTEM_MOVESIZESTART : EVENT_SYSTEM_MOVESIZEEND, hwnd,
            0, 0);
}
...
static int HitTestTimeoutbl(HWND hwnd, POINT pt) {
    ...
    if (hwnd != ancestor && blacklisted(ancestor, &BlkLst.NCHittest)) {
        SendMessageTimeout(ancestor, WM_NCHITTEST, 0, MAKELPARAM(pt.x, pt.y),
                           SMTO_NORMAL, 200, &area);
...
DOUBLECHECK_CAPTION:
    if (area == HTCAPTION) {
        ...
        RECT buttonRc;
        if (GetCaptionButtonsRect(ancestor, &buttonRc) &&
            PtInRect(&buttonRc, pt)) {
...
static void *GetEnoughSpace(void *ptr, unsigned num, unsigned *alloc,
                            size_t size) {
    if (num >= *alloc) {
...
BOOL CALLBACK EnumMonitorsProc(HMONITOR hMonitor, HDC hdcMonitor,
    ...
    monitors = (RECT *)GetEnoughSpace(monitors, nummonitors, &monitors_alloc,
                                      sizeof(RECT));
...
BOOL CALLBACK EnumWindowsProc(HWND window, LPARAM lParam) {
    ...
    if (ShouldSnapTo(window) && GetWindowRectL(window, &wnd)) {
        ...
        if (IsZoomed(window)) {
            ...
            // Crop this window so that it does not exceed the size of the
            // monitor This is done because when maximized, windows have an
            // extra invisible border (a border that stretches onto other
            // monitors)
...
BOOL CALLBACK EnumSnappedWindows(HWND hwnd, LPARAM lParam) {
    // Make sure we have enough space allocated
    snwnds = (struct snwdata *)GetEnoughSpace(snwnds, numsnwnds, &snwnds_alloc,
                                              sizeof(struct snwdata));
    ...
    if (ShouldSnapTo(hwnd) && !IsZoomed(hwnd) && GetWindowRectL(hwnd, &wnd)) {
        ...
        } else if ((restore = GetRestoreFlag(hwnd)) && restore & SNAPPED &&
                   restore & SNAPPEDSIDE) {
...
BOOL CALLBACK EnumTouchingWindows(HWND hwnd, LPARAM lParam) {
    ...
    RECT wnd;
    if (ShouldSnapTo(hwnd) && !IsZoomed(hwnd) && IsResizable(hwnd) &&
        !blacklisted(hwnd, &BlkLst.Windows) && GetWindowRectL(hwnd, &wnd)) {
...
static void EndDeferWindowPosAsync(HDWP hwndSS) {
    DWORD lpThreadId;
    CloseHandle(CreateThread(NULL, STACK, EndDeferWindowPosThread, hwndSS,
...
static int ResizeTouchingWindows(LPVOID lwptr) {
    ...
    for (i = 0; i < numsnwnds; i++) {
        ...
        if (!PtInRect(&state.origin.mon, tpt)) continue;
        ...
        if (PureLeft(flag)) {
            ...
            nwnd->left =
                CLAMP(nwnd->right - Max.x, posx + width, nwnd->right - Min.x);
        } else if (PureTop(flag)) {
            nwnd->bottom = posy;
        } else if (PureBottom(flag)) {
            POINT Min, Max;
            GetMinMaxInfo(hwnd, &Min, &Max);
            nwnd->top = CLAMP(nwnd->bottom - Max.x, posy + height,
                              nwnd->bottom - Min.x);
        } else {
            continue;
        }
        if (hwndSS) {
            RECT nbd;
            FixDWMRect(hwnd, &nbd);
            hwndSS = DeferWindowPos(
                hwndSS, hwnd, NULL, nwnd->left - nbd.left, nwnd->top - nbd.top,
    ...
    if (hwndSS) {
        ...
        hwndSS = DeferWindowPos(
            hwndSS, state.hwnd, NULL, lw->x, lw->y, lw->width, lw->height,
            SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
...
static void ResizeAllSnappedWindowsAsync() {
    ...
    for (i = 0; i < numsnwnds; i++) {
        if (hwndSS && snwnds[i].flag & TORESIZE) {
            ...
            hwndSS = DeferWindowPos(
                hwndSS, snwnds[i].hwnd, NULL, snwnds[i].wnd.left,
                snwnds[i].wnd.top, snwnds[i].wnd.right - snwnds[i].wnd.left,
    ...
    if (hwndSS)
        hwndSS = DeferWindowPos(
            hwndSS, LastWin.hwnd, NULL, LastWin.x, LastWin.y, LastWin.width,
            LastWin.height, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOOWNERZORDER);
    if (hwndSS) EndDeferWindowPosAsync(hwndSS);
    LastWin.hwnd = NULL;
}
    ...
    for (i = 0, j = 0; i < nummonitors || j < numwnds;) {
        ...
        if (IsInRangeT(posy, snapwnd.top, snapwnd.bottom, thresholdx) ||
            ...
            UCHAR snapinside_cond =
                (snapinside || posy + wndheight - thresholdx < snapwnd.top ||
                 snapwnd.bottom < posy + thresholdx);
            if (IsEqualT(snapwnd.right, posx, thresholdx)) {
                // The left edge of the dragged window will snap to this
                // window's right edge
                stuckx = 1;
                stickx = snapwnd.right;
                thresholdx = snapwnd.right - posx;
            } else if (snapinside_cond &&
                       IsEqualT(snapwnd.right, posx + wndwidth, thresholdx)) {
                // The right edge of the dragged window will snap to this
                // window's right edge
                stuckx = 1;
        ...
        if (IsInRangeT(posx, snapwnd.left, snapwnd.right, thresholdy) ||
            ...
            UCHAR snapinside_cond =
                (snapinside || posx + wndwidth - thresholdy < snapwnd.left ||
                 snapwnd.right < posx + thresholdy);
            if (IsEqualT(snapwnd.bottom, posy, thresholdy)) {
                // The top edge of the dragged window will snap to this window's
                // bottom edge
                stucky = 1;
                ...
                // The bottom edge of the dragged window will snap to this
        ...
        if (IsInRangeT(*posx, snapwnd.left, snapwnd.right, thresholdy) ||
            ...
            if (state.resize.y == RZ_TOP &&
                ...
                thresholdy = snapwnd.bottom - *posy;
                       ...
                       IsEqualT(snapwnd.bottom, *posy + *wndheight,
                                thresholdy)) {
                // The bottom edge of the dragged window will snap to this
                // window's bottom edge
                stuckbottom = 1;
...
static void MaximizeRestore_atpt(HWND hwnd, UINT sw_cmd, int origin) {
    ...
    if (sw_cmd != SW_FULLSCREEN) wndpl.showCmd = sw_cmd;
    ...
    if (sw_cmd == SW_MAXIMIZE || sw_cmd == SW_FULLSCREEN) {
        ...
        HMONITOR monitor =
            MonitorFromPoint(state.prevpt, MONITOR_DEFAULTTONEAREST);

        if (origin & 1) {
            // set normal windpw plasement to origin.
            wndpl.rcNormalPosition.right =
                wndpl.rcNormalPosition.left + state.origin.width;
            wndpl.rcNormalPosition.bottom =
                wndpl.rcNormalPosition.top + state.origin.height;
        }
...
#define r wndpl.rcNormalPosition
            ...
            MoveWindowAsync1(hwnd, r.left, r.top, r.right - r.left,
                             r.bottom - r.top);
#undef r
        }
    } else {
...
static void MoveResizeWindowThread(struct windowRR *lw, UINT flag) {
    ...
    if (lw->start) NotifySizeMoveStaEnd(hwnd, 1);  // Once darg actually starts.
    ...
    if (lw->end && !lw->maximize && (IsZoomed(hwnd) || IsWindowSnapped(hwnd))) {
        ...
        //        MAKELPARAM(state.prevpt.x, state.prevpt.y));
        //        if(!(flag&SWP_NOSIZE)) {
        //            RECT rc = { lw->x, lw->y, lw->x + lw->width, lw->y +
        //            lw->height }; SendMessage(hwnd, WM_SIZING,
        //            WMSZ_BOTTOMRIGHT, (LPARAM)&rc);
        //        }
    ...
    if (lw->end && !conf.FullWin) NotifySizeMoveStaEnd(hwnd, 0);
...
static DWORD WINAPI MoveWindowThread(LPVOID LastWinV) {
    ...
    //        }
    //        //LOGA("MV: %d:%d/%d -> %d:%d/%d %s", cW, cH, cdpi , lw->width,
    //        lw->height, lw->odpi, notsamesize?"(dif)":"(eq)");
    //    }
    //    UINT flag = notsamesize? RESIZEFLAG: state.resizable&2 ?
    //    MOVETHICKBORDERS: MOVEASYNC;
    int notsamesize = 1;
    int nothingtodo = 0;
    RECT rc;
    if (conf.FullWin) {
    ...
    UINT flag = notsamesize           ? RESIZEFLAG
                : state.resizable & 2 ? MOVETHICKBORDERS
                                      : MOVEASYNC;
...
static void MoveWindowInThread(struct windowRR *lw) {
    DWORD lpThreadId;
    CloseHandle(CreateThread(NULL, STACK, MoveWindowThread, lw,
                             STACK_SIZE_PARAM_IS_A_RESERVATION, &lpThreadId));
}
///////////////////////////////////////////////////////////////////////////
// use snwnds[numsnwnds].wnd / .flag
static void GetAeroSnappingMetrics(int *leftWidth, int *rightWidth,
...
static void WaitMovementEnd() {  // Only wait 64ms maximum
    if (conf.FullWin) {
        int i = 0;
...
static int AeroMoveSnap(POINT pt, int *posx, int *posy, int *wndwidth,
                        int *wndheight) {
    ...
    GetAeroSnappingMetrics(&leftWidth, &rightWidth, &topHeight, &bottomHeight,
                           &mon);
    ...
    if (pt.y < Top && pt.x < Left) {
        ...
        if (!state.shift ^ !(conf.AeroTopMaximizes & 1) &&
            ...
            if (conf.FullWin) {
                ...
                *wndheight = CLAMPH(mon.bottom - mon.top);
                ...
                SetRestoreFlag(
                    state.hwnd,
                    SNAPPED | SNCLEAR);  // To clear eventual snapping info
                return 0;
            }
        } else {
            ...
            *wndwidth = CLAMPW(mon.right - mon.left);
            *wndheight = topHeight;
            *posx = mon.left + (mon.right - mon.left) / 2 -
                    *wndwidth / 2;  // Center
            *posy = mon.top;
        }
        ...
        *wndheight = bottomHeight;
        *posx =
            mon.left + (mon.right - mon.left) / 2 - *wndwidth / 2;  // Center
        *posy = mon.bottom - *wndheight;
    } else if (pt.x < pLeft) {
        // Pure Left
        ...
        *posx = mon.left;
        *posy =
            mon.top + (mon.bottom - mon.top) / 2 - *wndheight / 2;  // Center
    } else if (pt.x > pRight) {
        // Pure Right
    ...
    if (restore & SNAPPED) {
        ...
        *wndheight = CLAMPH(*wndheight);
        ...
        SetRestoreData(state.hwnd, state.origin.width, state.origin.height,
                       restore);
        ...
        *wndheight += borders.top + borders.bottom;
        ...
        if (conf.FullWin) {
            if (IsZoomed(state.hwnd)) {
                // Avoids flickering
                RestoreWindowTo(state.hwnd, *posx, *posy, *wndwidth,
                                *wndheight);
...
///////////////////////////////////////////////////////////////////////////
static void AeroResizeSnap(POINT pt, int *posx, int *posy, int *wndwidth,
                           int *wndheight) {
    // return if last resizing is not finished
    if (!conf.Aero || MM_THREAD_ON || state.Speed > conf.AeroMaxSpeed) return;
    ...
    unsigned restore = GetRestoreFlag(state.hwnd);
    if (state.resize.x == RZ_XCENTER && state.resize.y == RZ_TOP &&
        pt.y < state.origin.mon.top + AERO_TH) {
        restore = SNAPPED | SNMAXH;
        *wndheight = CLAMPH(state.origin.mon.bottom - state.origin.mon.top +
                            borders.bottom + borders.top);
        *posy = state.origin.mon.top - borders.top;
    } else if (state.resize.x == RZ_LEFT && state.resize.y == RZ_YCENTER &&
               pt.x < state.origin.mon.left + AERO_TH) {
        restore = SNAPPED | SNMAXW;
        *wndwidth = CLAMPW(state.origin.mon.right - state.origin.mon.left +
...
static pure int IsAKeyDown(const UCHAR *k) {
    while (*k) {
        if (GetKeyState(*k++) & 0x8000) return 1;
    }
    return 0;
}
/////////////////////////////////////////////////////////////////////////////
...
static void RestoreOldWin(const POINT pt, unsigned was_snapped, RECT *ownd) {
    ...
    if (((rdata_flag & SNAPPED) &&
         !(state.origin.maximized && rdata_flag & 2))) {
    ...
    // Set offset
    state.offset.x = state.origin.width *
                     min(pt.x - wnd.left, wnd.right - wnd.left) /
                     max(wnd.right - wnd.left, 1);
    state.offset.y = state.origin.height *
                     min(pt.y - wnd.top, wnd.bottom - wnd.top) /
...
static void ShowTransWin(int nCmdShow) {
    if (conf.TransWinOpacity) {
        ...
        int i;
        for (i = 0; i < 4; i++)
            if (g_transhwnd[i]) ShowWindow(g_transhwnd[i], nCmdShow);
    }
}
...
static void MoveTransWin(int x, int y, int w, int h) {
    if (state.origin.dpi) {
        ...
        if (hmon != state.origin.monitor && !GetRestoreFlag(state.hwnd)) {
            ...
            if (S_OK == GetDpiForMonitorL(hmon, MDT_DEFAULT, &ptdpi,
                                          &dpiy_ignore) &&
                ptdpi) {
...
static void SetWindowPlacementThread(HWND hwnd, WINDOWPLACEMENT *wndplptr) {
    LastWin.hwnd = hwnd;
    DWORD lpThreadId;
    CloseHandle(CreateThread(NULL, STACK, WinPlacmntTrgead, wndplptr,
                             STACK_SIZE_PARAM_IS_A_RESERVATION, &lpThreadId));
}
...
static void MouseMove(POINT pt) {
    ...
    if (state.action == AC_MOVE) {
        ...
        if (state.resize.x == RZ_XCENTER && state.resize.y == RZ_YCENTER) {
            ...
            if (state.resize.y == RZ_TOP) {
                wndheight = CLAMPH((wnd.bottom - pt.y + state.offset.y) -
                                   state.mdipt.y);
            ...
            if (state.resize.x == RZ_LEFT) {
                wndwidth =
                    CLAMPW((wnd.right - pt.x + state.offset.x) - state.mdipt.x);
            ...
            // Check if the window will snap anywhere
            ResizeSnap(&posx, &posy, &wndwidth, &wndheight, conf.SnapThreshold,
                       conf.SnapThreshold);
    ...
    if (!conf.FullWin) {
        ...
        if (!IsTransWinVisible()) ShowTransWin(SW_SHOWNA);
        state.moving = 1;
...
#define Send_CTRL() \
    if (conf.EndSendKey) Send_KEY(conf.EndSendKey)
...
static DWORD WINAPI Send_ClickProc(LPVOID buttonD) {
    ...
    static const WORD bmapping[] = {MOUSEEVENTF_LEFTDOWN, MOUSEEVENTF_RIGHTDOWN,
...
static void Send_Click_Thread(enum button button) {
    DWORD id;
    CloseHandle(CreateThread(NULL, STACK, Send_ClickProc,
                             (LPVOID)(LONG_PTR)button,
...
static void SendUnicodeKey(WORD w) {
    KEYBDINPUT ctrl[2] = {{0, 0, KEYEVENTF_UNICODE, 0, 0},
...
DWORD WINAPI ActionKillThread(LPVOID hwnd) {
    if (state.shift) {
        ...
        // taskkill.exe /F /FI "status eq NOT RESPONDING" /FI "IMAGENAME ne
        // AltSnap.exe" /FI "IMAGENAME ne dwm.exe"
    } else {
        DWORD pid;
...
static int ActionKill(HWND hwnd) {
    ...
    if (blacklisted(hwnd, &BlkLst.Pause)) return 0;

    DWORD lpThreadId;
    CloseHandle(CreateThread(NULL, STACK, ActionKillThread, hwnd,
                             STACK_SIZE_PARAM_IS_A_RESERVATION, &lpThreadId));
...
static int ScrollLockState() {
    if (state.altsnaponoff) return 1;  // AltSnap was disabled by AC_ASONOFF
...
static void LogState(const char *Title) {
    ...
    fprintf(f,
            ...
            "blockmouseup=%d\n"
            "ignorekey=%d\n\n\n",
            (int)state.clickbutton, (DWORD)(DorQWORD)state.hwnd,
            (DWORD)(DorQWORD)LastWin.hwnd, (int)LastWin.end,
            (int)LastWin.moveonly, (int)LastWin.maximize, (int)LastWin.snap,
            (int)state.blockaltup, (int)state.blockmouseup,
            (int)state.ignorekey);
    fclose(f);
}
static pure int XXButtonIndex(UCHAR vkey) {
    ...
    for (i = 0; i < MAXKEYS && conf.XXButtons[i]; i++) {
        if (conf.XXButtons[i] == vkey) return i + 3;
    }
    return -1;
}
...
static void ToggleSnapState(void) { state.snap = state.snap ? 0 : 3; }
static void TogglesAlwaysOnTop(HWND hwnd);
static HWND MDIorNOT(HWND hwnd, HWND *mdiclient_);
static void ToggleClickThrough(HWND hwnd, int toggle);
///////////////////////////////////////////////////////////////////////////
...
#pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    //        , wParam, kbh->vkCode, kbh->scanCode, kbh->flags,
    //        kbh->dwExtraInfo);
    //    }
...
// Stupid software that like to steal Alt.
            ...
            if (conf.GrabWithAlt[0] || conf.GrabWithAlt[1]) {
                POINT pt;
                enum action action =
                    conf.GrabWithAlt[IsModKey(vkey) ||
                                     (!IsHotkey(conf.ModKey[0]) && ModKey())];
                if (action) {
            ...
            if (!state.shift && !IsModKey(vkey) /* != conf.ModKey*/) {
                ...
                state.shiftpt =
                    state.prevpt;  // Save point where shift was pressed.
        ...
        } else if (vkey == VK_SPACE && state.action &&
                   !IsSamePTT(&state.clickpt, &state.prevpt)) {
            ToggleSnapState();
            ...
            HotkeyUp();
        } else if (IsHotkeyy(vkey, conf.ESCkeys)) {  // USER PRESSED ESCAPE!
                                                     // (vkey == VK_ESCAPE)
        ...
        } else if (!state.ctrl && state.alt != vkey &&
                   (vkey == VK_LCONTROL || vkey == VK_RCONTROL) &&
                   !(kbh->scanCode &
                     SCANCODE_SIMULATED) /* Ignore ALT GR Scan Code (&0x0200) */
        ...
        } else if (state.sclickhwnd && g_mchwnd && state.alt &&
                   (vkey == VK_LMENU || vkey == VK_RMENU)) {
            // Block Alt down when the altsnap's menu just opened
            if (state.unikeymenu == (HMENU)1 ||
                (IsWindow(state.sclickhwnd) && IsWindow(g_mchwnd) &&
                 IsMenu(state.unikeymenu)))
                return 1;
        } else if ((xxbtidx = XXButtonIndex(vkey)) >= 0 &&
                   (GetAction(BT_MMB + xxbtidx) ||
                    GetActionT(BT_MMB + xxbtidx) ||
                    IsHotclick(BT_MMB + xxbtidx))) {
            if (!state.xxbutton) {
        ...
        } else if (conf.UniKeyHoldMenu && (fhwnd = GetForegroundWindow()) &&
                   !IsFullScreenBL(fhwnd) &&
                   !blacklisted(fhwnd, &BlkLst.Processes) &&
                   !blacklisted(fhwnd, &BlkLst.Windows)) {
            // Key lists used below...
            static const UCHAR ctrlaltwinkeys[] = {VK_CONTROL, VK_MENU, VK_LWIN,
                                                   VK_RWIN, 0};
            static const UCHAR menupopdownkeys[] = {
                VK_BACK,  VK_TAB,   VK_APPS, VK_DELETE, VK_SPACE, VK_LEFT,
                VK_RIGHT, VK_PRIOR, VK_NEXT, VK_END,    VK_HOME,  0};
            if (state.unikeymenu && IsMenu(state.unikeymenu) &&
                !(GetKeyState(vkey) & 0x8000)) {
                if (vkey == VK_SNAPSHOT)
                    return CallNextHookEx(NULL, nCode, wParam, lParam);
                ...
                PostMessage(g_mchwnd, WM_KEYDOWN, vkey,
                            0);  // all keys are "directed to the Menu"
                return 1;        // block keydown

            } else if (!state.ctrl && !state.alt &&
                       (0x41 <= vkey && vkey <= 0x5A) &&
                       !IsAKeyDown(ctrlaltwinkeys)) {
                ...
                if (GetKeyState(vkey) & 0x8000) {  // The key is autorepeating.
                    if (!IsMenu(state.unikeymenu)) {
                        ...
                        g_mchwnd = KreateMsgWin(
                            MenuWindowProc, TEXT(APP_NAMEA) TEXT("-SClick"), 2);
                        UCHAR shiftdown = GetKeyState(VK_SHIFT) & 0x8000 ||
                                          GetKeyState(VK_CAPITAL) & 1;
                        PostMessage(g_mainhwnd, WM_UNIKEYMENU, (WPARAM)g_mchwnd,
                                    vkey | (shiftdown << 8));
                    }
                    return 1;  // block keydown
                }
...
static int ScrollPointedWindow(POINT pt, int delta, WPARAM wParam) {
    ...
    if (!hwnd || blacklisted(hwnd, &BlkLst.IScroll)) return 0;

    HWND foreground = GetForegroundWindow();

    // Return if foreground window is blacklisted
    if (foreground && blacklisted(foreground, &BlkLst.Windows)) return 0;
...
// lParam means to include minimized windows (pass TRK_LASERMODE to
// TrackMenuOfWindows)
BOOL CALLBACK EnumAltTabWindows(HWND window, LPARAM lParam) {
    ...
    if (IsAltTabAble(window) &&
        ...
        state.origin.monitor ==
            MonitorFromWindow(window, conf.MenuShowOffscreenWin
...
/////////////////////////////////////////////////////////////////////////////
static pure BOOL StackedRectsT(
    const RECT *a, const RECT *b,
    const int T) {  // Determine wether or not the windows are stacked...
...
BOOL CALLBACK EnumStackedWindowsProc(HWND hwnd, LPARAM lasermode) {
    ...
    // Only store window if it's visible, not minimized to taskbar
    RECT wnd, refwnd;
    if (IsAltTabAble(hwnd) && !IsIconic(hwnd) &&
        GetWindowRectL(state.hwnd, &refwnd) && GetWindowRectL(hwnd, &wnd) &&
...
/////////////////////////////////////////////////////////////////////////////
static int ActionAltTab(POINT pt, short delta, short laser,
    ...
    if (conf.MDI) {
        ...
        if (mdiclient) {
            ...
            if (numhwnds > 1) {
                if (delta > 0) {
                    PostMessage(mdiclient, WM_MDIACTIVATE,
                                (WPARAM)hwnds[numhwnds - 1], 0);
                } else {
...
static HRESULT(WINAPI *myCoCreateInstance)(REFCLSID rclsid, LPUNKNOWN pUnkOuter,
...
static HRESULT GetCurrentVolumeMute(UINT *curentVol, UINT *maxVol,
                                    BOOL *muted) {
    ...
    if (osver >= 6 && LoadOLEDLLOnce()) {
        ...
        hr =
            myCoCreateInstance(&my_CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                               &my_IID_IMMDeviceEnumerator, (void **)&pDevEnum);
        if (hr != S_OK) goto fail;

        hr = pDevEnum->lpVtbl->GetDefaultAudioEndpoint(pDevEnum, eRender,
                                                       eMultimedia, &pDev);
        pDevEnum->lpVtbl->Release(pDevEnum);
        if (hr != S_OK) goto fail;

        hr = pDev->lpVtbl->Activate(pDev, &my_IID_IAudioEndpointVolume,
                                    CLSCTX_ALL, NULL, (void **)&pAudioEndp);
        pDev->lpVtbl->Release(pDev);
        if (hr != S_OK) goto fail;

        hr = pAudioEndp->lpVtbl->GetVolumeStepInfo(pAudioEndp, curentVol,
                                                   maxVol) |
             pAudioEndp->lpVtbl->GetMute(pAudioEndp, muted);

        pAudioEndp->lpVtbl->Release(pAudioEndp);

        LOG("%d==GetVolumeStepInfo() -> %u (0 - %u) Muted=%d ", hr, curentVol,
            maxVol, muted);
...
static void ActionVolume(int delta) {
    ...
    BOOL muted = FALSE;
    if (S_OK == GetCurrentVolumeMute(&curentvol, &maxvol, &muted) &&
        curentvol) {
        if (delta < 0 && (UINT)num >= curentvol) {
            ...
            if (!muted) Send_KEY(VK_VOLUME_MUTE);  // Mute
            return;
        } else if (delta > 0) {
            if (muted) {
...
static void SetBottomMost(HWND hwnd) {
    ...
    if (!blacklisted(hwnd, &BlkLst.Bottommost)) {
        ...
        while ((tmph = GetNextWindow(tmph, GW_HWNDNEXT)) && i++ < 1024) {
            if (IsVisible(tmph) && !IsIconic(tmph) &&
...
static void ActionMaxRestMin(HWND hwnd, int delta) {
    ...
    if (delta > 0) {
        if (!maximized && IsResizable(hwnd)) MaximizeWindow(hwnd);
    } else {
        if (maximized)
            RestoreWindow(hwnd);
        else
...
// Works oly for Desktop monitors.
    ...
    BOOL(WINAPI * myGetPhysMonitorsFromHM)(HMONITOR hMonitor, DWORD sz,
                                           LPPHYSICAL_MONITOR pmarr);
    BOOL(WINAPI * myGetMonitorBrightness)(HANDLE hMonitor, LPDWORD min,
                                          LPDWORD cur, LPDWORD max);
    BOOL(WINAPI * mySetMonitorBrightness)(HANDLE hMonitor,
                                          DWORD dwNewBrightness);
    BOOL(WINAPI * myDestroyPhysicalMonitors)(DWORD cnt,
                                             LPPHYSICAL_MONITOR pmarr);
    BOOL(WINAPI * myGetNumberOfPhysmons)(HMONITOR hMonitor,
                                         LPDWORD pdwNumberOfPhysicalMonitors);
    BOOL(WINAPI * myGetMonitorCapabilities)(HANDLE hMonitor, LPDWORD supcap,
                                            LPDWORD supcoltemp);
    ...
    if (dll) {
        ...
        myGetPhysMonitorsFromHM =
            (BOOL(WINAPI *)(HMONITOR, DWORD, LPPHYSICAL_MONITOR))GetProcAddress(
                dll, "GetPhysicalMonitorsFromHMONITOR");
        myGetMonitorBrightness =
            (BOOL(WINAPI *)(HANDLE, LPDWORD, LPDWORD, LPDWORD))GetProcAddress(
                dll, "GetMonitorBrightness");
        mySetMonitorBrightness = (BOOL(WINAPI *)(HANDLE, DWORD))GetProcAddress(
            dll, "SetMonitorBrightness");
        myDestroyPhysicalMonitors =
            (BOOL(WINAPI *)(DWORD, LPPHYSICAL_MONITOR))GetProcAddress(
                dll, "DestroyPhysicalMonitors");
        myGetNumberOfPhysmons =
            (BOOL(WINAPI *)(HMONITOR, LPDWORD))GetProcAddress(
                dll, "GetNumberOfPhysicalMonitorsFromHMONITOR");
        myGetMonitorCapabilities =
            (BOOL(WINAPI *)(HANDLE, LPDWORD, LPDWORD))GetProcAddress(
                dll, "GetMonitorCapabilities");
        ...
        LOG("Physical Monitor=%x, %ls", pm->hPhysicalMonitor,
            pm->szPhysicalMonitorDescription);

        ok =
            myGetMonitorCapabilities(pm->hPhysicalMonitor, &dwMCap, &wdColtemp);
        ...
        if (ok && MC_CAPS_BRIGHTNESS & dwMCap) {
            ...
            LOG("Brightness of %ls: min=%lu, cur=%lu, max=%lu",
            ...
            if (ok) {
                ...
                LOG("SetMonitorBrightness(%x, %d)=%d",
                    (UINT)(UINT_PTR)pm->hPhysicalMonitor, newbr, ok);
            }
        }

        ok = myDestroyPhysicalMonitors(numpm, pm);
        LOG("DestroyPhysicalMonitors(%lu, %x)=%d", numpm, (UINT)(UINT_PTR)pm,
            ok);
        free(pm);
...
static HCURSOR CursorToDraw() {
    ...
    if (state.action == AC_MOVE) {
        if (conf.UseCursor == 4) return LoadCursor(NULL, IDC_SIZEALL);
...
static void RollWindow(HWND hwnd, int delta) {
    ...
    if (restore & ROLLED && delta <= 0) {  // UNROLL
        ...
        if (!(restore & ROLLED)) {  // Save window size if not saved already.
            ...
            // Add the SNAPPED falg is maximized and and add the SNTHENROLLED
            // flag is snapped
...
static int ActionZoom(HWND hwnd, short delta, short center) {
    ...
    if (!conf.AutoSnap ||
        (CT && (state.resize.x == RZ_XCENTER ||
                state.resize.y ==
                    RZ_YCENTER))) {  // Try to conserve a bit better the aspect
                                     // ratio when in center mode.
        T = 1;                       // Or when no snapping has to occur.
    }

    GetMinMaxInfo(hwnd, &state.mmi.Min,
                  &state.mmi.Max);  // for CLAMPH/W functions
    ...
    int x = rc.left, y = rc.top, width = rc.right - rc.left,
...
static int IsDoubleClick(
    int button) {  // Never validate a double-click if the click has to pierce
...
static int ActionMove(POINT pt, int button) {
    ...
    if (IsDoubleClick(button)) {
        ...
        if (state.shift) {
            ...
            state.blockmouseup = 1;  // Block the mouseup, otherwise it can
    ...
    } else if (conf.DragThreshold >= 2 ||
               (conf.DragThreshold == 1 && IsAnySnapped(state.hwnd))) {
...
static void SetEdgeAndOffsetCF(const RECT *wnd, const UCHAR centerfrac,
                               POINT pt) {
    // Set edge and offset
    // Think of the window as nine boxes (corner regions get 38%, middle only
    // 24%) Does not use state.origin.width/height since that is based on
    // wndpl.rcNormalPosition which is not what you see when resizing a window
    // that Windows Aero resized
    ...
    if (pt.x - wnd->left < (wndwidth * SideS) / 100) {
        ...
        state.offset.x =
            pt.x - state.mdipt.x;  // Used only if both x and y are CENTER
    } else {
        state.resize.x = RZ_RIGHT;
...
static void SnapToCorner(HWND hwnd, struct resizeXY resize, UCHAR flags) {
    ...
    SetEdgeAndOffset(&wnd,
                     state.prevpt);  // state.resize.x/y & state.offset.x/y
    if (!resize.x && !resize.y) resize = state.resize;
    FixDWMRect(hwnd, &bd);
    int wndwidth = wnd.right - wnd.left;
    int wndheight = wnd.bottom - wnd.top;
    ...
    if (flags & SNTO_EXTEND) {
        ...
        if (resize.y == RZ_TOP) {
            ...
            wndheight =
                CLAMPH(mon->bottom - wnd.top + state.mdipt.y + bd.bottom);
        }
        ...
        int leftWidth, rightWidth, topHeight, bottomHeight;
        // EnumSnapped();
        GetAeroSnappingMetrics(&leftWidth, &rightWidth, &topHeight,
                               &bottomHeight, mon);
        wndwidth = leftWidth;
        wndheight = topHeight;
        posx = mon->left;
    ...
    if (!(GetRestoreFlag(hwnd) & SNAPPED))
        SetRestoreData(hwnd, state.origin.width, state.origin.height,
                       SNAPPED | restore);
}
...
static int ActionResize(POINT pt, const RECT *wnd, int button) {
    ...
    if (IsDoubleClick(button)) {
        ...
        SnapToCorner(state.hwnd, AUTORESIZE,
                     !state.shift ^ !(conf.AeroTopMaximizes & 2));
...
static void ActionBorderless(HWND hwnd) {
    ...
    GetWindowRect(hwnd, &rc);
    SetWindowPos(
        hwnd, NULL, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top + 1,
...
static void CenterWindow(HWND hwnd, unsigned flags) {
    ...
    // extra calculation to centre the portion that is inside the invisible
    // borders
    int x = mon.left +
            ((mon.right - mon.left) - (width - bd.left - bd.right)) / 2 -
            bd.left;
    int y = mon.top +
            ((mon.bottom - mon.top) - (height - bd.top - bd.bottom)) / 2 -
            bd.top;
...
static void CALLBACK HandleWinEvent(HWINEVENTHOOK hook, DWORD event, HWND hwnd,
    ...
    if (hwnd &&
        ...
        idChild == INDEXID_CONTAINER && idObject == OBJID_WINDOW) {
...
static LRESULT CALLBACK PinWindowProc(HWND hwnd, UINT msg, WPARAM wp,
                                      LPARAM lp) {
    switch (msg) {
        case WM_CREATE: {
#ifdef EVENT_HOOK
...
#endif
                BYTE *thunk = (BYTE *)VirtualAlloc(NULL, THUNK_SIZE, MEM_COMMIT,
                                                   PAGE_READWRITE);
                if (!thunk) break;
...
#error Unsupported CPU type, implement thunking or undefine EVENT_HOOK.
                ...
                HWINEVENTHOOK hook = SetWinEventHook(
                    ...
                    // Range of events=8001-800Bh
                    0, EVENT_OBJECT_LOCATIONCHANGE,
                    NULL,                 // Handle to DLL.
                    (WINEVENTPROC)thunk,  // The callback function (thunked)
                    lpdwProcessId,
                    threadid,  // Process and thread IDs of interest (0 = all)
                    WINEVENT_OUTOFCONTEXT  // Flags.
                );
                SetWindowLongPtr(hwnd, 0 + sizeof(LONG_PTR),
                                 (LONG_PTR)hook);  // save hook
                SetWindowLongPtr(hwnd, 0 + 2 * sizeof(LONG_PTR),
                                 (LONG_PTR)thunk);  // save thunk
                if (hook) {
        ...
        case WM_TIMER: {
            ...
            if (!IsWindow(ow) || !IsWindowVisible(ow) ||
                !((xstyle = GetWindowLongPtr(ow, GWL_EXSTYLE)) &
                  WS_EX_TOPMOST)) {
                DestroyWindow(hwnd);
                return 0;
            }
            ...
            if (OldOwStyle == style) {
                ...
                LONG_PTR RTOffset =
                    GetWindowLongPtr(hwnd, 0);  // get Right and Top offset
                SetWindowPos(hwnd, NULL, rc.right - LOWORDPTR(RTOffset),
                             rc.top + HIWORDPTR(RTOffset), 0, 0,
            ...
            PinW =
                GetSystemMetricsForDpi(SM_CXSIZE, dpi);  // Caption button width
            PinH = GetSystemMetricsForDpi(SM_CYSIZE,
                                          dpi);  // Caption button height
            ...
            SetWindowLongPtr(hwnd, GWLP_USERDATA,
                             style);  // Save old window style
            int rightoffset = CapButtonWidth + PinW + bdx +
                              GetSystemMetricsForDpi(SM_CXBORDER, dpi) * 2;
            int topoffset = bdy + 1;
            // Cache local hwnd storage for pin win offsets in the first
            // LONG_PTR of cbWndExtra
            SetWindowLongPtr(hwnd, 0, MAKELONGPTR(rightoffset, topoffset));
            // Move and size the window...
            SetWindowPos(hwnd, NULL, rc.right - rightoffset, rc.top + topoffset,
                         PinW, PinH,
                         SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER |
                             SWP_NOSENDCHANGING);
            return 0;
...
#ifdef EVENT_HOOK
            HWINEVENTHOOK hook =
                (HWINEVENTHOOK)GetWindowLongPtr(hwnd, 0 + sizeof(LONG_PTR));
            if (hook) UnhookWinEvent(hook);
            // Free the thunk.
            BYTE *thunk =
                (BYTE *)GetWindowLongPtr(hwnd, 0 + 2 * sizeof(LONG_PTR));
...
static void MaximizeHV(HWND hwnd, int horizontal) {
    ...
    if (horizontal) {
        ...
        MoveWindowAsync(hwnd, mon.left - bd.left, rc.top,
                        mon.right - mon.left + bd.left + bd.right,
                        rc.bottom - rc.top);
    } else {  // vertical
        SetRestoreFlag(hwnd, SNAPPED | SNMAXH);
        MoveWindowAsync(hwnd, rc.left, mon.top - bd.top, rc.right - rc.left,
...
BOOL CALLBACK MinimizeWindowProc(HWND hwnd, LPARAM lParam) {
    minhwnds = (HWND *)GetEnoughSpace(minhwnds, numminhwnds, &minhwnds_alloc,
                                      sizeof(HWND));
    if (!minhwnds) return FALSE;  // Stop enum, we failed
    const struct MinimizeWindowProcParams *p =
        (const struct MinimizeWindowProcParams *)lParam;
    hwnd = GetRootOwner(hwnd);

    if (hwnd != p->clickedhwnd
        // && IsAltTabAble(hwnd) ????
        && IsVisible(hwnd) && !IsIconic(hwnd) &&
    ...
    if (restore == hwnd) {
        ...
        for (i = 0; i < numminhwnds; i++) {
            ...
            if (IsWindow(hrest) && IsIconic(hrest) &&
                (!hMon ||
                 hMon == MonitorFromWindow(hrest, MONITOR_DEFAULTTONEAREST))) {
...
static pure BOOL IsRectInMonitors(const RECT *rc) {
    ...
    for (i = 0; i < nummonitors; i++) {
        int inx = monitors[i].left < rc->right - 8 &&
                  rc->left + 8 < monitors[i].right;
        int iny = monitors[i].top < rc->bottom - 8 &&
                  rc->top + 8 < monitors[i].bottom;
...
static void TrackMenuOfWindows(WNDENUMPROC EnumProc, LPARAM flags) {
    ...
    for (i = 0; i < numhwnds; i++) {
        ...
        if (conf.MaxMenuWidth) textlen = min(textlen, conf.MaxMenuWidth);
...
// Fill up MSAA structure for screen readers.
    ...
    if (0 < i && i <= numhwnds) {
        ...
        if (IsIconic(hwnd)) RestoreWindow(hwnd);
        SetForegroundWindowL(hwnd);

        if (flags & TRK_MOVETOMONITOR) MoveToCurrentMonitorIfNeeded(hwnd);
    }
    ...
    // Free strings
    for (i = 0; i < numhwnds; i++) {
        if (data[i].txtptr != failed_string) free(data[i].txtptr);
    }
...
static void ActionStackList(int lasermode) {
    ...
    PostMessage(g_hkhwnd, WM_STACKLIST, lasermode,
                (LPARAM)EnumStackedWindowsProc);
}
...
static BOOL CALLBACK FindTiledWindowEnumProc(HWND hwnd, LPARAM lp) {
    ...
    long adx = abs(dx);
    long ady = abs(dy);
    // LOGA("adx = %d, ady = %d, tw->opt=%d,%d tw->oheight = %d, tw->owidth =
    // %d"
    ...
    if (tw->diagonal) {
        ...
        switch (tw->direction) {
            case 0:  // LEFT
                if (dx < 0 && ady <= adx &&
                    (adx < tw->distance.x ||
                     (adx == tw->distance.x && ady < tw->distance.y)))
                    break;    // Window is closer...
                return TRUE;  // skip
            case 1:           // UP
                if (dy < 0 && adx <= ady &&
                    (ady < tw->distance.y ||
                     (ady == tw->distance.y && adx < tw->distance.x)))
                    break;
                return TRUE;
            case 2:  // RIGHT
                if (dx > 0 && ady <= adx &&
...
static HWND FindTiledWindow(HWND hwnd, unsigned char direction) {
    ...
    if (GetWindowRectL(hwnd, &rc)) {
        ...
        //            tw.oheight = i*h;
        //            tw.owidth  = i*h;
        //            EnumDesktopWindows(NULL, FindTiledWindowEnumProc,
        //            (LPARAM)&tw);
        //        }
        //        // TODO: Handle MDI clients.
        //        if (state.mdiclient) {
        //            EnumChildWindows(state.mdiclient, FindTiledWindowEnumProc,
        //            (LPARAM)&tw);
        //        } else {
        //            EnumDesktopWindows(NULL, FindTiledWindowEnumProc,
        //            (LPARAM)&tw);
...
static void SClickActions(HWND hwnd, enum action action) {
    ...
    switch (action) {
        ...
        case AC_ALWAYSONTOP:
            TogglesAlwaysOnTop(hwnd);
            break;
        case AC_CLICKTHROUGH:
            ToggleClickThrough(hwnd, 1);
            break;
        case AC_CLOSE:
            PostMessage(hwnd, WM_SYSCOMMAND, SC_CLOSE, 0);
            break;
        ...
        case AC_EXTENDTNEDGE:
            SnapToCorner(hwnd, AUTORESIZE,
                         state.shift ? SNTO_MOVETO | SNTO_NEXTBD
                                     : SNTO_EXTEND | SNTO_NEXTBD);
            break;
        case AC_MOVETNEDGE:
            SnapToCorner(hwnd, AUTORESIZE,
                         state.shift ? SNTO_EXTEND | SNTO_NEXTBD
                                     : SNTO_MOVETO | SNTO_NEXTBD);
            break;
        case AC_MENU:
            ActionMenu(hwnd);
            break;
        case AC_NSTACKED:
            ActionAltTab(state.prevpt, +120, state.shift,
                         EnumStackedWindowsProc);
            break;
        case AC_NSTACKED2:
            ActionAltTab(state.prevpt, +120, !state.shift,
                         EnumStackedWindowsProc);
        ...
        case AC_STACKLIST2:
            ...
            break;
        case AC_ALTTABLIST:
            PostMessage(g_hkhwnd, WM_STACKLIST,
                        TRK_MOVETOMONITOR | TRK_LASERMODE,
                        state.shift ? (LPARAM)EnumAllAltTabWindows
                                    : (LPARAM)EnumAltTabWindows);
            break;
        case AC_ALTTABFULLLIST:
            PostMessage(g_hkhwnd, WM_STACKLIST,
                        TRK_MOVETOMONITOR | TRK_LASERMODE,
                        state.shift ? (LPARAM)EnumAltTabWindows
                                    : (LPARAM)EnumAllAltTabWindows);
            break;
        case AC_MLZONE:
        ...
        default:
            // Shortcuts 0 - 35
            if (AC_SHRT0 <= action &&
                action < AC_SHRT0 + ARR_SZ(conf.inputSequences) &&
...
static int DoWheelActions(HWND hwnd, enum action action) {
    ...
    switch (action) {
        case AC_ALTTAB:
            ActionAltTab(
                state.prevpt, state.delta, /*laser=0*/ 0,
        ...
        case AC_HSCROLL:
            ret =
                ScrollPointedWindow(state.prevpt, -state.delta, WM_MOUSEHWHEEL);
            break;
        case AC_ZOOM:
        ...
        case AC_ZOOM2:
            ...
            break;
        case AC_NPSTACKED:
            ActionAltTab(state.prevpt, state.delta, state.shift,
                         EnumStackedWindowsProc);
            break;
        case AC_NPSTACKED2:
            ActionAltTab(state.prevpt, state.delta, !state.shift,
                         EnumStackedWindowsProc);
            break;
            //    case AC_BRIGHTNESS:   ActionBrightness(state.prevpt,
            //    state.delta); break;
        default: {
            ret = 0;  // No action
            ...
            if (AC_SHRT0 <= rac &&
                rac < AC_SHRT0 + ARR_SZ(conf.inputSequences) &&
...
static int xpure DoubleClamp(int ptx, int left, int right,
                             int rwidth) {  // Same logic applies to y axis
    int posx;
...
static int init_movement_and_actions(POINT pt, HWND hwnd, enum action action,
    ...
    if (blacklisted(state.hwnd, &BlkLst.Processes) ||
        ...
        (blacklisted(state.hwnd, &BlkLst.Windows) && !state.hittest &&
         button != BT_WHEEL &&
         button != BT_HWHEEL)  // does not apply in titlebar, nor for the wheel
                               // action...
        ...
        ((state.origin.fullscreen =
              IsFullscreenF(state.hwnd, &wnd, &mi.rcMonitor)) &&
         !conf.FullScreen)) {
        return 0;
    }
    ...
    GetMinMaxInfo(state.hwnd, &state.mmi.Min,
                  &state.mmi.Max);  // for CLAMPH/W functions
    ...
    if (MOUVEMENT(action)) {
        ...
        if (conf.DragSendsAltCtrl && !(GetKeyState(VK_MENU) & 0x8000) &&
            ...
            CloseHandle(CreateThread(NULL, STACK, SendAltCtrlAlt, 0,
        ...
        if (GetRestoreData(state.hwnd, &rwidth, &rheight) & SNAPPED) {
            ...
            GetWindowRect(state.hwnd, &rc);
            int posx =
                DoubleClamp(pt.x, rc.left, rc.right, rwidth) - state.mdipt.x;
            int posy =
                DoubleClamp(pt.y, rc.top, rc.bottom, rheight) - state.mdipt.y;
...
static int xpure IsAeraCapbutton(int area) {
    return area == HTMINBUTTON || area == HTMAXBUTTON || area == HTCLOSE ||
           area == HTHELP;
}
...
static int InTitlebar(POINT pt, enum action action, enum button button) {
    ...
    if (willtest && action) {
        ...
        if (area == HTCAPTION  // Real caption
            || (!(conf.BLCapButtons & (1 << (button - 2))) &&
                IsAreaAnyCap(area)) ||
            (!(conf.BLUpperBorder & (1 << (button - 2))) &&
             IsAreaTopRZ(area))) {
            return area;
        }
    }
...
// static void FinishMovement() { PostMessage(g_hkhwnd, WM_FINISHMOVEMENT, 0,
// 0); }
static void FinishMovement() {
    LOG("FinishMovement");
    // LogState("");
    ...
    if (conf.AutoRemaximize && state.moving &&
        (state.origin.maximized || state.origin.fullscreen) && !state.shift &&
    ...
    if (conf.FullWin && state.moving == 1) NotifySizeMoveStaEnd(state.hwnd, 0);
...
static void DoComboActions(enum action action, enum button button) {
    // For safety
    if (!MOUVEMENT(state.action)) return;
    ...
    if (button == BT_WHEEL || button == BT_HWHEEL) {
        ...
        if (accombo) {
            ...
            if (ClickComboActions(action)) return;
...
static xpure int GetButton(WPARAM wp, LPARAM lp) {
    ...
    return (wp == WM_LBUTTONDOWN || wp == WM_LBUTTONUP)   ? BT_LMB
           ...
           : (wp == WM_XBUTTONDOWN || wp == WM_XBUTTONUP)
               ? BT_MB4 - 1 + HIWORD(msg->mouseData)
               : BT_NONE;
}
static xpure enum buttonstate GetButtonState(WPARAM wp) {
    return (wp == WM_LBUTTONDOWN || wp == WM_MBUTTONDOWN ||
            wp == WM_RBUTTONDOWN || wp == WM_XBUTTONDOWN)
               ? STATE_DOWN
           : (wp == WM_LBUTTONUP || wp == WM_MBUTTONUP || wp == WM_RBUTTONUP ||
              wp == WM_XBUTTONUP)
               ? STATE_UP
...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
    ...
    POINT pt = msg->pt;
    //    if ((0x201 > wParam || wParam > 0x205) && wParam != 0x20a && wParam !=
    //    WM_MOUSEMOVE)
    //        LOGA("wParam=%lx, data=%lx, time=%lu, extra=%lx, block?=%d,
    //        ignored?=%d", (DWORD)wParam
    //            , (DWORD)msg->mouseData, (DWORD)msg->time,
    //            (DWORD)msg->dwExtraInfo , (int)state.blockmouseup,
    ...
    if (wParam == WM_MOUSEMOVE) {
        ...
        if (state.action && !state.blockmouseup) {  // resize or move...
            ...
            if (conf.RezTimer == 1) {
                ...
                updaterate =
                    (updaterate + 1) %
                    (state.action == AC_MOVE ? conf.MoveRate : conf.ResizeRate);
                if (!updaterate) {
                    MouseMove(pt);
    ...
    //    if (button<=BT_MB5)
    //        LOGA("button=%d, %s", button,
    //        buttonstate==STATE_DOWN?"DOWN":buttonstate==STATE_UP?"UP":"NONE");
    ...
    if (buttonstate == STATE_DOWN && state.action &&
    ...
    if (!state.alt && is_hotclick && buttonstate == STATE_DOWN) {
        ...
        // if (!action) return 1;
        if (!action && (conf.AblockHotclick || state.sactiondone)) return 1;
    ...
    if (buttonstate == STATE_DOWN && state.alt) {
        ...
        if (!state.action && !IsHotclick(state.alt) && !IsHotkeyDown()) {
            UnhookMouse();
            return CALLNEXTHOOK;  // CallNextHookEx(NULL, nCode, wParam,
                                  // lParam);
        }
        ...
        if (action &&
            (state.action == action ||
             (state.action == AC_MOVE && action == AC_RESIZE)) &&
            (!state.moving || state.moving == DRAG_WAIT)  // No drag occured
            &&
            state.sactiondone <=
                AC_RESIZE  // Only move/resize may have happened in the meantime
            ...
            int inTTB =
                2 * (!!state.hittest);  // If we are in the titlebar add two
...
static void UnhookMouseOnly() {
    ...
    if (!mousehook || conf.keepMousehook || state.blockmouseup) return;
...
static xpure int IsAreaLongClikcable(int area) {
    return IsAeraCapbutton(area) || area == HTHSCROLL || area == HTVSCROLL ||
           area == HTSYSMENU;
}
/////////////////////////////////////////////////////////////////////////////
// Window for timers only...
LRESULT CALLBACK TimerWindowProc(HWND hwnd, UINT msg, WPARAM wParam,
                                 LPARAM lParam) {
    if (msg == WM_TIMER) {
        switch (wParam) {
#ifndef NO_HOOK_LL
            case REHOOK_TIMER: {
                // Silently rehook hooks if they have been stopped (>= Win7 and
                // LowLevelHooksTimeout) This can often happen if locking or
                // sleeping the computer a lot
                POINT pt;
                GetCursorPos(&pt);  // I donot know if we should really use the
                                    // ASYN version.
            ...
            case SPEED_TIMER: {
                ...
                if (state.moving)
                    state.Speed = max(abs(oldpt.x - state.prevpt.x),
                                      abs(oldpt.y - state.prevpt.y));
                else
                    state.Speed = 0;
                oldpt = state.prevpt;
                if (state.moving && state.Speed == 0 &&
                    !has_moved_to_fixed_pt && !MM_THREAD_ON) {
            ...
            case GRAB_TIMER: {
                ...
                if (IsSamePTT(&pt, &state.clickpt) &&
                    GetAsyncKeyState(1 + (buttonswaped = !!GetSystemMetrics(
                                              SM_SWAPBUTTON))) &&
                    (ptwnd = WindowFromPoint(pt)) &&
                    !IsAreaLongClikcable(HitTestTimeoutbl(ptwnd, pt))) {
                    // Determine if we should actually move the Window by
                    // probing with AC_NONE
                    ...
                    if (ret) {  // Release mouse click if we have to move.
                        InterlockedIncrement(&state.ignoreclick);
                        mouse_event(buttonswaped ? MOUSEEVENTF_RIGHTUP
                                                 : MOUSEEVENTF_LEFTUP,
            ...
            case POOL_TIMER: {
                ...
                if (nmsll.time == omsll.time) return 0;

                if (!SamePt(nmsll.pt, omsll.pt))
                    LowLevelMouseProc(HC_ACTION, WM_MOUSEMOVE, (LPARAM)&nmsll);
                if (nbts[0] != obts[0])
                    LowLevelMouseProc(HC_ACTION,
                                      nbts[0] ? WM_LBUTTONDOWN : WM_LBUTTONUP,
                                      (LPARAM)&nmsll);
                if (nbts[1] != obts[1])
                    LowLevelMouseProc(HC_ACTION,
                                      nbts[1] ? WM_RBUTTONDOWN : WM_RBUTTONUP,
                                      (LPARAM)&nmsll);
                if (nbts[2] != obts[2])
                    LowLevelMouseProc(HC_ACTION,
                                      nbts[2] ? WM_MBUTTONDOWN : WM_MBUTTONUP,
                                      (LPARAM)&nmsll);
...
static LRESULT DrawMenuItem(HWND hwnd, WPARAM wParam, LPARAM lParam, UINT dpi,
    ...
    Rectangle(di->hDC, di->rcItem.left, di->rcItem.top, di->rcItem.right + 1,
              di->rcItem.bottom + 1);
    ...
    // LOGA("WM_DRAWITEM: txtXY=%u, %u, txt=%S", (UINT)sz.cx, (UINT)sz.cy,
    // data->txtptr);

    int totheight =
        di->rcItem.bottom - di->rcItem.top;     // total menuitem height
    int yicooffset = (totheight - yicosz) / 2;  // Center icon vertically
    int ytxtoffset = (totheight - sz.cy) / 2;   // Center text vertically

    DrawIconEx(
        di->hDC, di->rcItem.left + xmargin, di->rcItem.top + yicooffset,
        (di->itemState & ODS_GRAYED) ? LoadIcon(NULL, IDI_HAND) : data->icon,
        xicosz, yicosz, 0, NULL, DI_NORMAL);
    if (IsIconic(hwnds[di->itemID - 1])) {
        ...
        MoveToEx(di->hDC, di->rcItem.left + xmargin,
                 di->rcItem.top + yicooffset + yicosz - 1, NULL);
        LineTo(di->hDC, di->rcItem.left + xmargin + xicosz,
               di->rcItem.top + yicooffset + yicosz - 1);
    ...
    //        MoveToEx(di->hDC, di->rcItem.left+xmargin,        di->rcItem.top +
    //        yicooffset, NULL); LineTo(di->hDC, di->rcItem.left+xmargin+xicosz,
    //        di->rcItem.top + yicooffset+yicosz); MoveToEx(di->hDC,
    //        di->rcItem.left+xmargin, di->rcItem.top + yicooffset+yicosz,
    //        NULL); LineTo(di->hDC,   di->rcItem.left+xmargin+xicosz,
...
static void SendSYSCOMMANDToMenuItem(HWND hwnd, int id, HMENU hmenu,
                                     WPARAM sc_command) {
    if (conf.RCCloseMItem && 0 <= id && (UINT)id < numhwnds &&
        GetWindowLongPtr(hwnd, GWLP_USERDATA) == 3 && IsWindow(hwnds[id])) {
...
LRESULT CALLBACK MenuWindowProc(HWND hwnd, UINT msg, WPARAM wParam,
                                LPARAM lParam) {
    static UINT dpi;
    ...
    switch (msg) {
        ...
        case WM_COMMAND: {
            if (LOWORD(wParam)) {
                ...
                // LOG("Unikey menu WM_COMMAND, wp=%X, lp=%X", (UINT)wParam,
                // (UINT)lParam);
                Send_KEY(VK_BACK);  // Errase old char...

                // Send UCS-2 or Lower+Upper UTF-16 surrogates of the UNICODE
                // char.
                ...
                // ACTION MENU LOWORD(wParam) has to be zero to differenctiae
                // with unikey menu
                LOG("Action Menu WM_COMMAND, wp=%X, lp=%X", (UINT)wParam,
                    (UINT)lParam);
        ...
        case WM_MEASUREITEM:
            ...
            //        USHORT id = (UINT) LOWORD(wParam); // identifier of the
            //        menu item USHORT fuFlags = (UINT) HIWORD(wParam); if
            //        (fuFlags == 0xFFFF && !lParam) {
            //            ShowTransWin(SW_HIDE);
            //        } else if (fuFlags&MF_HILITE && fuFlags&MF_OWNERDRAW
            //        && 0 < id && id <= numhwnds) {
            //            HWND selhwnd = hwnds[id-1]; // currently selected
            //            hwnd. RECT rc; GetWindowRectL(selhwnd, &rc);
            //            ShowTransWin(SW_SHOWNA);
            //            MoveTransWin(rc.left, rc.top, rc.right-rc.left,
            //            rc.bottom-rc.top);
            //        }
            //        } break;
        ...
        case WM_MENUCHAR: {
            ...
            if (GetWindowLongPtr(hwnd, GWLP_USERDATA) == 3) {
                ...
                if (conf.NumberMenuItems) {
                    ...
                    // A-Z then 0-9
                    // If UPPERCASE
                    closewindow = conf.RCCloseMItem && 'A' <= cc && cc <= 'Z' &&
                                  GetKeyState(VK_SHIFT) & 0x8000;
                    item = ('a' <= c && c <= 'z')   ? c - 'a'
                           : ('0' <= c && c <= '9') ? c - '0' + 26
                                                    : 0xFFFF;
                ...
                if (item != 0xFFFF && item <= numhwnds) {
                    if (closewindow)
                        ...
                        SendSYSCOMMANDToMenuItem(hwnd, item, (HMENU)lParam,
                                                 SC_CLOSE);
                    else
        ...
        case WM_DESTROY: {
            ...
            if (fhwndori && state.sclickhwnd == fhwndori &&
...
LRESULT CALLBACK HotKeysWinProc(HWND hwnd, UINT msg, WPARAM wParam,
                                LPARAM lParam) {
    if (msg == WM_HOTKEY) {
        int actionint = 0;
        int ptwindow = 0;
        ...
        if (action > AC_RESIZE) {  // Exclude resize action in case...
            POINT pt;
            GetMsgPT(&pt);
            static const enum action noinitactions[] = {
                AC_KILL, AC_PAUSE, AC_RESUME, AC_ASONOFF, AC_NONE};
            if (IsActionInList(action, noinitactions)) {
                // Some actions pass directly through the default blacklists...
                HWND targethwnd =
                    ptwindow ? WindowFromPoint(pt) : GetForegroundWindow();
                ...
                if (!ptwindow) {
                    ...
                    static const enum action resetPTaclist[] = {
                        AC_MENU, AC_NSTACKED, AC_PSTACKED, AC_NONE};
                    if (IsActionInList(action, resetPTaclist)) {
                        state.ignorept = 1;
                    }
                    // Might be of use for something?
                    //} else if (IsActionInList(action, ptCenterAcList)) {
                    //    // We must select the center of the current window as
                    //    the point
                ...
                state.blockmouseup =
                    0;  // We must not block mouseup in this case...
...
#pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
    ...
    EnumThreadWindows(GetCurrentThreadId(), PostPinWindowsProcMessage,
                      WM_CLOSE);
...
static void readblacklist(const TCHAR *section, struct blacklist *blacklist,
                          const char *bl_str) {
    ...
    while (pos) {
        ...
        if (title) {
            ...
            if (klass) {
                if (klass < title) {
                    // if a ':' comes after a '|' the there is a ':' in the
                    // class and exename is not specified ie: title|class
                    // format. We do this because there can be no '|' in a
                    // filename
                    title = exenm;
                    exenm = NULL;
                } else {
        ...
        blacklist->items = (struct blacklistitem *)realloc(
            blacklist->items,
            (blacklist->length + 1) * sizeof(struct blacklistitem));
        if (!blacklist->items) {
...
void readallblacklists(const TCHAR *inipath) {
    ...
    if (!section) return;
    GetPrivateProfileSection(TEXT("Blacklist"), section,
                             blacklist_section_length, inipath);
...
static unsigned readhotkeys(const TCHAR *inisection, const char *name,
                            const TCHAR *def, UCHAR *keys, unsigned MaxKeys) {
...
void readbuttonactions(const TCHAR *inputsection) {
    static const char *buttons[] = {
        "LMB",         "RMB",    "MMB",      "MB4",     "MB5",  "MB6",
        "MB7",         "MB8",    "MB9",      "MB10",    "MB11", "MB12",
        "MB13",        "MB14",   "MB15",     "MB16",    "MB17", "MB18",
        "MB19",        "MB20",   "Scroll",   "HScroll",
    ...
    for (i = 0; i < ARR_SZ(buttons); i++) {
        enum action *const actionptr =
            &conf.Mouse.LMB[0];  // first action in list
...
static HWND KreateMsgWin(WNDPROC proc, const TCHAR *name, LONG_PTR userdata) {
    ...
    HWND parent =
        (LOBYTE(LOWORD(GetVersion())) >= 5) ? HWND_MESSAGE : g_mainhwnd;
    HWND hwnd = CreateWindowEx(0, wnd.lpszClassName, NULL, 0, 0, 0, 0, 0,
                               parent, NULL, hinstDLL, NULL);
    if (hwnd && userdata) SetWindowLongPtr(hwnd, GWLP_USERDATA, userdata);
    return hwnd;
}
...
static void CreateTransWin(const TCHAR *inisection) {
    ...
    if (conf.TransWinOpacity) {
        ...
        g_transhwnd[0] =
            CreateWindowEx(xflags, wnd.lpszClassName, NULL, WS_POPUP, 0, 0, 0,
                           0, NULL, NULL, hinstDLL, NULL);
        if (conf.TransWinOpacity != 255)
            SetLayeredWindowAttributes(g_transhwnd[0], 0, conf.TransWinOpacity,
                                       LWA_ALPHA);
    }
    if (!g_transhwnd[0]) {
        int i;
        for (i = 0; i < 4;
             i++) {  // the transparent window is made with 4 thin windows
            g_transhwnd[i] = CreateWindowEx(
                WS_EX_TOPMOST | WS_EX_TOOLWINDOW, wnd.lpszClassName, NULL,
                WS_POPUP, 0, 0, 0, 0, g_mainhwnd, NULL, hinstDLL, NULL);
...
void registerAllHotkeys(const TCHAR *inipath) {
    g_hkhwnd =
        KreateMsgWin(HotKeysWinProc, TEXT(APP_NAMEA) TEXT("-HotKeys"), 0);
    ChangeWindowMessageFilterExL(g_hkhwnd, WM_HOTKEY, /*MSGFLT_ALLOW*/ 1, NULL);
    // MOD_ALT=1, MOD_CONTROL=2, MOD_SHIFT=4, MOD_WIN=8
    // RegisterHotKey(g_hkhwnd, 0xC000 + AC_KILL,   MOD_ALT|MOD_CONTROL, VK_F4);
    // // F4=73h Read All shortcuts in the [KBShortcuts] section.
    TCHAR inisection[1800];
    GetPrivateProfileSection(TEXT("KBShortcuts"), inisection,
                             ARR_SZ(inisection), inipath);
...
#undef ACVALUE
    ...
    for (ac = AC_MENU; ac < ARR_SZ(action_names); ac++) {
        ...
        if (LOBYTE(HK) && HIBYTE(HK)) {
            ...
            if (!RegisterHotKey(g_hkhwnd, 0xC000 + ac, HIBYTE(HK),
                                LOBYTE(HK))) {
                LOG("Error registering hotkey %s=%x", action_names[ac],
                    (unsigned)HK);
#ifdef LOG_STUFF
                TCHAR title[76], acN[32];
                lstrcpy_s(title, ARR_SZ(title),
                          TEXT(APP_NAMEA)
                              TEXT(": unable to register hotkey for action "));
...
static void readalluchars(UCHAR *dest, const TCHAR *const inisection,
                          const struct OptionListItem *optlist,
                          size_t listlen) {
    // Read all char options
    unsigned i;
    for (i = 0; i < listlen; i++) {
        *dest++ =
            GetSectionOptionInt(inisection, optlist[i].name, optlist[i].def);
    }
}
void readallinputSequences(const TCHAR *inisection) {
    ...
    for (i = 0; i < ARR_SZ(conf.inputSequences); i++) {
        ...
        unsigned len =
            readhotkeys(inisection, shrtN, TEXT(""), buf + 1, 508) / 2;
        buf[0] = len;
        if (len) {
...
extern "C"
#endif
    __declspec(dllexport) HWND WINAPI
    Load(HWND mainhwnd, const TCHAR inipath[AT_LEAST MAX_PATH]) {
#if defined(_MSC_VER) && _MSC_VER > 1300
#pragma comment(linker, "/EXPORT:" __FUNCTION__ "=" __FUNCDNAME__)
#endif
    // Load settings
    ...
    if (SystemParametersInfo(/*SPI_GETMOUSEDRAGOUTTHRESHOLD*/ 0x0084, 0,
                             &dragthreshold, 0)) {
    ...
    // [General]
    GetPrivateProfileSection(TEXT("General"), inisection, inisectionlen,
                             inipath);
    readalluchars(&conf.AutoFocus, inisection, General_uchars,
                  ARR_SZ(General_uchars));
    ...
    // [Advanced]
    GetPrivateProfileSection(TEXT("Advanced"), inisection, inisectionlen,
                             inipath);
    readalluchars(&conf.ResizeAll, inisection, Advanced_uchars,
                  ARR_SZ(Advanced_uchars));
    ...
    conf.LongClickMoveDelay =
        GetSectionOptionInt(inisection, "LongClickMoveDelay", 0);
    if (conf.LongClickMoveDelay == 0)
        conf.LongClickMoveDelay = GetDoubleClickTime();

    GetPrivateProfileSection(TEXT("Performance"), inisection, inisectionlen,
                             inipath);
    readalluchars(&conf.FullWin, inisection, Performance_uchars,
                  ARR_SZ(Performance_uchars));
    ...
    if (conf.RezTimer == 2 || conf.RezTimer == 4) {
        ...
        if (EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &dvm)) {
            ...
            if (dvm.dmDisplayFrequency == 60) conf.RezTimer = 1;
        }
    }
    if (conf.RezTimer)
        conf.RefreshRate = 0;  // Ignore the refresh rate in RezTimer mode.
    if (conf.FullWin ==
        2) {  // Use current config to determine if we use FullWin.
    ...
    readalluchars(&conf.TTBActions, inisection, Input_uchars,
                  ARR_SZ(Input_uchars));
    readbuttonactions(inisection);

    if (conf.TopmostIndicator) {
        int color[2];
        readhotkeys(inisection, "PinColor", TEXT("FF FF 00 54"),
                    (UCHAR *)&color[0], 4);
    ...
    for (i = 0; i < ARR_SZ(hklst); i++) {
        readhotkeys(inisection, hklst[i].name, hklst[i].def,
                    &conf.Hotkeys[i * (MAXKEYS + 1)], MAXKEYS);
    }
    ...
    readalluchars(&conf.UseZones, inisection, Zones_uchars,
                  ARR_SZ(Zones_uchars));
    ...
    if (inisection != stk_inisection) free(inisection);
    ...
    if (conf.keepMousehook || conf.AeroMaxSpeed < 65535) {
        g_timerhwnd =
            KreateMsgWin(TimerWindowProc, TEXT(APP_NAMEA) TEXT("-Timers"), 0);
    }
